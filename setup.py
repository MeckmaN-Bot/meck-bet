...
def collect_api_key() -> str:
  ...
  {bold('Action Network')} (default, {green('FREE, no key needed')}):
    Real-time NBA odds from DraftKings, FanDuel, BetMGM, Caesars & more.
    No account required — recommended for most users.

  {bold('The Odds API')} (optional):
    Broader coverage incl. Pinnacle & EU books. Free tier: 500 req/month.
    Only needed if you want EU bookmakers or non-NBA sports.
    Sign up: https://the-odds-api.com
    """).rstrip())

    key = ask("The Odds API key (Enter = use Action Network)")
    if not key:
        ok("Using Action Network — no API key needed.")
        return ""
    ...
    if ask_yn("Use Action Network instead?", default=True):
        ok("Using Action Network.")
        return ""
...
def write_env(cfg: dict):
    ...
    odds_line = (
        f"ODDS_API_KEY={cfg['api_key']}"
        if cfg['api_key']
        else "# ODDS_API_KEY=  # Not set — using Action Network (free)"
    )
...
# ── Odds Source ───────────────────────────────────────────────────────────────
# Action Network is used automatically (free, no key needed).
# Set ODDS_API_KEY to also use The Odds API (optional, broader coverage).
{odds_line}
...
def main():
    ...
    section("Summary")
    hr()
    odds_src = green("The Odds API") if api_key else cyan("Action Network (free)")
    print(f"  {'Odds source':<22} {odds_src}")
